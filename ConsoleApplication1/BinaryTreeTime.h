...
namespace Lab
{
	...
	private:
		...
		static void InsertTime()
		{
			...
			for (int i = 1; i <= 1000000; i++)
			{
				...
				tree = tree->InsertNode(value, IntComparer);
				if (i % 10000 == 0)
				{
					...
					Console::WriteLine("Inserted {0} nodes, time: {1}", i, seconds);
				}
			}
		}
		...
		static void FindTime(int start, int end, int step, int operations)
		{
			for (int nodes = start; nodes <= end; nodes += step)
			{
				...
				Console::WriteLine("Inserted {0} nodes", nodes);
				...
				for (int avg = 0; avg < 10; avg++)
				{
					...
					{
						int value = rand->Next(0, 10000);
						tree->Find(tree, value, IntComparer);
					}
					int endTime = System::Environment::TickCount;
					int ticks = endTime - startTime;
					sum += ticks;
				...
				int avgTime = sum / 10;
				Console::WriteLine("Avg find node time: {0}", avgTime);
			}
		}
		...
		static void RemoveTime(int start, int end, int step, int operations)
		{
			for (int nodes = start; nodes <= end; nodes += step)
			{
				...
				int avgTime = sum / 10;
				Console::WriteLine("Avg remove node time: {0}", avgTime);
			}
		}
	...
	public:
		static void Run()
		{
			...
			RemoveTime(10000, 100000, 5000, 1000);
		}
	};
}